...
- [x] Figure out how to deal with grid layouts? (A Flex is greedy, there is no way to arrange things in a grid)
- [ ] Dropbox
- [ ] Pre-rendering expensive components -> provide some pre-render hook that returns a texture. Does that mean that scene renderer should just render to a texture instead of the screen (already does it partially) and then a different final renderer renders the result? Or should I just be able to specify the render target? Then I could simply defer to the scene renderer for pre-rendering. Other idea is to, instead of rebuilding the scene on every render call, to rebuild it partially. How do we detect this? We would need to somehow detect which elements want to change in size and then only redraw elements that fall within that size. Would this be an extra parameter in the ui context? something like "scissor rect"? Would still mean that the entire layout has to reflow... unless i have a smarter cache invalidation strategy, one that takes more than just size into account, like state changes, i don't think i have a choice. I could have elements emit / signal to the cache check functions to see if something is still valid... but then we need to make the validity of the parents also conditional on the validity of the children...