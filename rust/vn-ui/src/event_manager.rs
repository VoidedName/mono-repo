...
impl EventManager {
    ...
    pub fn queue_event(&mut self, event: InteractionEvent) {
        self.event_queue.push(event);
    }

    pub fn process_events(&mut self) -> Vec<(ElementId, InteractionEvent)> {
        ...
        for event in queue {
            match event {
                ...
                InteractionEvent::Keyboard(key_event) => {
                    all_events.extend(self.handle_key(&key_event));
                }
                // These are usually generated by handle_mouse_move/down/up
                _ => {}
            }
        }
